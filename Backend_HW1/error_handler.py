...
async def bad_request(send):
    ...
    await send({"type": "http.response.body", "body": json.dumps({"error": "400 Bad Request"}).encode("utf-8")})
...
async def unprocessable_entity(send):
    ...
    await send({"type": "http.response.body", "body": json.dumps({"error": "422 Unprocessable Entity"}).encode("utf-8")})
